...
class RegistrationController extends AppController
{
    ...
    {
        if ($_POST['email'] == null or $_POST['password'] == null or $_POST['name'] == null or $_POST['surname'] == null or $_POST['place'] == null or $_POST['phone'] == null)
            return $this->render('registration', ['messages' => ['Należy wypełnić wszystkie pola']]);
        if ($_POST['password'] != $_POST['repeatedPassword']) {
            return $this->render('registration', ['messages' => ['Błędnie powtórzono hasło']]);
        }
        if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
            return $this->render('registration', ['messages' => ['Błędny adres email']]);
        }
        if (!$this->isPost()) {
        ...
        if ($user != null) {
            return $this->render('registration', ['messages' => ['Użytkownik z tym emailem istnieje']]);
        }
